...
{
public:
	ResourceManager(void(*deleter)(T*));
	~ResourceManager();
...
template<typename T>
ResourceManager<T>::ResourceManager(void(*deleter)(T*))
{
	m_deleter = deleter;
}
...
template<typename T>
inline std::string ResourceManager<T>::getResourceName(T * resource)
{
	for (auto it = m_resources.begin(); it != m_resources.end(); ++it)
	{
...
template<typename T>
bool ResourceManager<T>::saveResource(const std::string& name, T* resource)
{
...
template<typename T>
bool ResourceManager<T>::returnResource(T* resource, bool deleteIfUnused)
{
	for (auto it = m_resources.begin(); it != m_resources.end(); ++it)
	{
		if (it->second == resource)

...
MotionPatternPoint::MotionPatternPoint(float normTimeMS, float x, float y, float normDepth, int triggerID, bool shouldFire, std::string animationName, bool shouldAnimationLoop)
	: normalizedTime(normTimeMS), spritePosition(Vector2(x, y)), normalizedDepth(normDepth), action{ triggerID, shouldFire, animationName, shouldAnimationLoop }
{
